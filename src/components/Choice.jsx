/** 선택 칩. 고르면 BE optional 로 넘어가고 "잘 모르겠어요"(빈 값)면 보내지 않는다 —
    그래야 missingInputs 안내가 그대로 남는다(원칙 5-①). 누르는 면적은 44px 을 지킨다. */
export function Choice({ label, active, onClick, wide }) {
  ...
  return (
    <button type="button" onClick={onClick} aria-pressed={active}
            className={`${base} ${tone} ${wide ? 'w-full rounded-xl px-5 py-3.5 text-left' : 'rounded-full px-5 py-2.5'}`}>
      {label}
    </button>
  );
}
...
export function ChoiceGroup({ label, hint, options, value, onChange, stack }) {
  return (
    <div className="mt-8">
      {label && <p className="mb-3 text-sm font-bold text-ink-soft">{label}</p>}
      {hint && <p className="mb-3 text-sm leading-relaxed text-muted">{hint}</p>}
