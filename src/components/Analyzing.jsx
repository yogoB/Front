...
export default function Analyzing({ onDone }) {
  ...
  return (
    <section aria-live="polite" className="mx-auto my-16 max-w-[460px] text-center">
      <span className="inline-grid size-12 place-items-center rounded-xl bg-brand-tint" aria-hidden="true">
        ...
        {STEPS.map((text, i) => (
          <li key={text}
              className={`rounded-[10px] border px-3.5 py-3 text-sm transition-opacity
                ${i < done ? 'border-brand-tint text-ink opacity-100' : 'border-line text-muted opacity-50'}`}>
            {i < done && <span className="font-extrabold text-brand">✓ </span>}{text}
          </li>
        ))}
      </ul>
