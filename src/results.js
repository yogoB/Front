// 결과 비교표. 목업: '현재'는 입력값 단순 합계(허용된 메모), '추천·최저'는 예시 값.
// ponytail: mockRecommendation은 데모용. 운영에선 POST /api/v1/recommendations 응답으로 대체한다.
...
function mockRecommendation(input) {
  ...
  const lowTotal = recTotal;                                   // Figma: 최저 = 추천
  return { subs, currentTotal, recTotal, lowTotal, firstSub };
}
