      ...
      <main className="mx-auto flex w-full max-w-page flex-1 flex-col px-6">
        <section className="flex flex-1 flex-col items-center justify-center gap-8 py-14 text-center md:py-20">
          ...
          {stats?.monthlyAverage != null
            ? <SavingsAverage average={stats.monthlyAverage} median={stats.monthlyMedian} basis={stats.basis} />
            : stats?.samples?.length > 0 && <SavingsTicker samples={stats.samples} basis={stats.basis} />}
...
/* 이용자 1인당 평균 절감액. 평균·중앙값 전부 BE 값이다 — 화면은 나누지도 더하지도 않는다(절대 원칙 2).
   표본 수는 적지 않는다(사용자 결정 2026-09-20): 지금은 노출 임계값이 2라 "2명 기준 평균"이 사실상
   그 두 사람의 금액을 알려 주는 꼴이 된다. 기준 문구는 남긴다(절대 원칙 4). */
function SavingsAverage({ average, median, basis }) {
  return (
    <div className="grid gap-2">
      ...
      <p className="text-[13px] leading-relaxed text-muted">
        {BASIS[basis] ?? '월 절감액'} · 진단에서 확인한 절감액 · 로그인하고 결과를 확인한 이용자 기준(계정당 1건)
...
function SavingsTicker({ samples, basis }) {
  ...
  return (
    <div className="grid gap-2">
      ...
      <p className="text-[13px] text-muted">
        {BASIS[basis] ?? '월 절감액'} · 로그인하고 결과를 확인한 이용자 표본(계정당 1건)
      </p>
    </div>
  );
}
