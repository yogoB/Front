...
function row(sub) {
  ...
  price.className = 'sub-price'; price.textContent = won(tier.price);
...
function renderSummary() {
  ...
  const subTotal = chosen.reduce((sum, s) => sum + s.service.tiers.find(t => t.id === s.tierId).price, 0);
  ...
  $('sum-subs').textContent = chosen.length ? won(subTotal) : '—';
}
...
function analyze() {
  ...
  const input = {
    ...
    subs: chosen.map(s => {
      ...
      return { id: s.id, name: s.service.name, tierId: t.id, tierName: t.name, price: t.price };
    }),
  };
