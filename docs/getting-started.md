...
| `npm test` | 로직을 고친 뒤. 커밋·배포 전에는 **항상** 돌린다 |
| `npm run build` | 배포용 파일을 `dist/`에 만든다. 커밋 전 한 번 돌려 깨지지 않는지 본다 |
| `npm run preview` | 만들어진 `dist/`를 실제 배포처럼 띄워 확인 |

### 배포 전 확인 (2026-09-18 사고 이후 규칙)

빌드와 테스트가 둘 다 통과한 채로 흰 화면이 운영에 나간 적이 있다. 원인은 지운 컴포넌트를
다른 컴포넌트가 계속 부르고 있던 것 — `vite build` 도 계약 테스트도 "쓰는데 없는 이름"을 보지 않았다.
`tests/components-defined.test.js` 가 이 부류를 막지만, 머지처럼 남의 변경이 섞인 뒤에는 그것만 믿지 않는다.

1. `npm test` — 전부 통과해야 한다.
2. `npm run build`.
3. **화면을 직접 연다.** 최소 `/`, `/modes`, `/light`, `/detail`, `/results`, `/calendar`, `/mypage`, `/admin`.
   흰 화면과 콘솔 오류는 여기서만 드러난다. 라우트를 한 번씩 열어 보는 데 1분이면 된다.
