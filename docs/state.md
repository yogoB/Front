...
기준일: **2026-09-20**. 이 문서는 작업 트리의 현재 구현 상태를 요약한다. API 계약과 화면별 동작은 [연동 명세](integration.md), 실행 방법은 [실행 안내](../README.md), 화면·흐름 정책은 [UX 흐름](ux-flow.md), 재편성 전 프로토타입 분석은 [이전 분석](README.md)을 본다.
...
| 주소 | 파일 | 로그인 | 상태 |
| --- | --- | --- | --- |
| `/` | `Landing.jsx` | — | 민트·블랙·화이트 단일 히어로. 실제 1인 절감액 표본을 무작위 룰렛으로 순환, 출처 툴팁 제공 |
| `/modes` | `Modes.jsx` | — | 간편/상세 선택 |
| `/light` `/detail` | `Light.jsx` `Detail.jsx` | — | 입력 단계. 현재 요금제 전체 검색·없는 이름 직접 반영, 무제한 필수 조건 적용 |
