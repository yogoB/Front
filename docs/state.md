...
번들 HTML 프로토타입을 재편성하고 브라우저 고정 계산을 제거해 `BE_main` API의 금액·순서·출처를 그대로 표시한다. 2026-09 전면 **Vite 8 + React 19 + React Router 7 + Tailwind CSS 4** 로 전환했다. `npm test` 31개 통과, 배포본은 https://yogob.fly.dev/ 이다.
...
`npm test` 31개. 계약·공개/회원 호출·입력 검증·검색·CSRF·오류·취소·일정 계산과 화면 회귀 규칙을 검사한다.
