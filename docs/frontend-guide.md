...
`tests/contract.test.js`가 **절대 규칙이 지켜지는지**를 검사한다: 요청이 계약대로 나가는지, 봉투 처리, 정수 검증, CSRF/쿠키, 오류 보존, CSV 안전성, 프론트 금액 계산 금지, Google 캘린더 링크에 금액이 실리지 않는지, CSRF 면제 경로가 아닌 변경 요청이 `member: true` 로 나가는지 등. 현재 **18개**가 모두 통과해야 한다.
...
- [ ] `npm test` 18개 통과 + `npm run build` 성공
